...
function ProductCard({ product, currencyCode, onAdd }: { product: Product; currencyCode: string; onAdd: (variant: Variant) => void }) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border bg-card shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-lg">
      ...
      <div className="p-3.5">
        <h2 className="line-clamp-2 min-h-10 text-sm font-semibold leading-5 tracking-tight">{product.name}</h2>
        <div className="mt-3 flex items-end justify-between gap-3">
          <div><p className="text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">From</p><p className="mt-0.5 text-lg font-bold tabular-nums text-slate-950 dark:text-white">{formatCurrency(lowestPrice, currencyCode)}</p></div>
          <span className="text-right text-[11px] text-muted-foreground">{variants.length} {variants.length === 1 ? "option" : "options"}</span>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <Button aria-label={`Add ${product.name} to cart`} className="shrink-0 bg-orange-600 text-white shadow-sm hover:bg-orange-700" disabled={!selectedVariant || selectedVariant.stock < 1} onClick={() => selectedVariant && onAdd(selectedVariant)} size="sm" type="button"><Plus aria-hidden="true" /> Add</Button>
        </div>
      </div>
    </article>
  );
